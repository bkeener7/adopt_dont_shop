...
RSpec.describe Applicant, type: :model do
  ...
  describe 'self.pet_applications' do
    it 'counts the number of pets an applicant has applied for' do
      shelter = Shelter.create!(name: 'Aurora shelter', city: 'Aurora, CO', foster_program: false, rank: 9)
      pet_1 = Pet.create!(adoptable: true, age: 1, breed: 'sphynx', name: 'Lucille Bald', shelter_id: shelter.id)
      pet_2 = Pet.create!(adoptable: true, age: 3, breed: 'doberman', name: 'Lobster', shelter_id: shelter.id)
      ...
      expect(applicant2.pet_applications).to eq([pet_application1, pet_application2])
      expect(applicant3.pet_applications).to eq([pet_application3])
    end
  end
end
